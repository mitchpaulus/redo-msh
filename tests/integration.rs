...
//! every platform (important for the cross-platform goal). Each test drives the
//! real `redo-msh` binary, which spawns real `msh` do-files, which call back
//! into `redo-msh` — exercising the full process tree.
//!
//! Requires `msh` on PATH.
...
impl Project {
    ...
    /// Number of lines in a log file (0 if absent).
    ...
    /// Run `redo-msh <args>` in the project with the binary's dir on PATH (so
    /// do-files can invoke `redo-msh`), plus any extra env.
    fn redo_env(&self, args: &[&str], env: &[(&str, &str)]) -> Output {
        ...
        cmd.args(args).current_dir(&self.dir).env("PATH", path);
        for (k, v) in env {
            cmd.env(k, v);
        }
        cmd.output().expect("failed to spawn redo-msh")
    }
...
/// `touch`-ing a source (no content change) does not trigger a rebuild.
#[test]
...
fn introspection() {
    ...
    let sources = String::from_utf8_lossy(&p.redo(&["sources"]).stdout).into_owned();
    let targets = String::from_utf8_lossy(&p.redo(&["targets"]).stdout).into_owned();
    ...
    let ood = String::from_utf8_lossy(&p.redo(&["ood"]).stdout).into_owned();
