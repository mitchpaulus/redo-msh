...
/// (env `REDO_YES=1`) we overwrite; on a tty we ask; otherwise we fail and
...
fn check_not_hand_edited(ctx: &Ctx, target_rel: &str, target_abs: &Path) -> Result<()> {
    ...
    if std::env::var("REDO_YES").as_deref() == Ok("1") {
        eprintln!("redo-msh: {problem}\nredo-msh: overwriting {target_rel} (--yes)");
        return Ok(());
    }
    if std::io::stderr().is_terminal() && prompt_overwrite(&problem)? {
        return Ok(());
    }
    bail!(
        "{problem}\n\
...
/// Ask on the controlling terminal whether to overwrite a target redo does
/// not own (hand-edited, or never generated by redo).
fn prompt_overwrite(problem: &str) -> Result<bool> {
