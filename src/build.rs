...
fn check_not_hand_edited(ctx: &Ctx, target_rel: &str, target_abs: &Path) -> Result<()> {
    ...
    if std::io::stderr().is_terminal() && prompt_overwrite(target_rel, problem)? {
        return Ok(());
    }
    bail!(
...
fn prompt_overwrite(target_rel: &str, problem: &str) -> Result<bool> {
    ...
    write!(tty, "redo-msh: {target_rel} {problem}. Overwrite? [y/N] ")?;
    tty.flush()?;
